...
- **Android**: Fix false `EVENT_PAYMENT_ERROR` (cancelled) when hardware back button or swipe-back gesture is pressed after payment is already deducted. The plugin now tracks payment completion via analytics events and suppresses cancel callbacks when a payment has already succeeded, with a 2-second grace period for late-arriving success events.
- **iOS**: Fix back button staying active and re-triggering cancel dialog after checkout dismisses. Added `paymentCompleted` flag and presentation cleanup that dismisses any lingering view controllers after payment success/error callbacks.
- **iOS**: Added `pendingReply` mechanism to `resync` for parity with Android, so lost responses can be retrieved when the Flutter engine reconnects.
